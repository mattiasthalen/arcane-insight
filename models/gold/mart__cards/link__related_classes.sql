/* Link between fact and dimension for all Hearthstone related classes */
MODEL (
  name gold.mart__cards.link__related_classes,
...
WITH fact__cards AS (
...
), dim__related_classes AS (
  SELECT
    *
  FROM gold.mart__cards.dim__related_classes
), fact__unnested AS (
  SELECT
    fact_record_hk,
    fact__extracted_at,
    ...
    fact__valid_from,
    fact__valid_to,
    UNNEST(multi_class_ids) AS related_class_id
  FROM fact__cards
), fact__aggregated AS (
  SELECT
    fact_record_hk,
  ...
  SELECT
    ...
    dim__related_classes.related_class_pit_hk, /* Unique identifier in time for the related class */
  ...
  LEFT JOIN dim__related_classes
    ON fact__aggregated.related_class_id = dim__related_classes.related_class_id
    AND fact__aggregated.link__loaded_at BETWEEN dim__related_classes.related_class__valid_from AND dim__related_classes.related_class__valid_from
)
SELECT
  *
FROM final
