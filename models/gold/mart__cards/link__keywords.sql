/* Link between fact and dimension for all Hearthstone keywords */
MODEL (
  name gold.mart__cards.link__keywords,
...
WITH fact__cards AS (
  ...
  SELECT
    *
  FROM gold.mart__cards.dim__keywords
), fact__unnested AS (
  SELECT
    fact_record_hk,
    fact__extracted_at,
    ...
    fact__valid_from,
    fact__valid_to,
    UNNEST(keyword_ids) AS keyword_id
  FROM fact__cards
), fact__aggregated AS (
  SELECT
    fact_record_hk,
  ...
  SELECT
    ...
    dim__keywords.keyword_pit_hk, /* Unique identifier in time for the keyword */
  ...
  LEFT JOIN dim__keywords
    ON fact__aggregated.keyword_id = dim__keywords.keyword_id
    AND fact__aggregated.link__loaded_at BETWEEN dim__keywords.keyword__valid_from AND dim__keywords.keyword__valid_from
)
SELECT
  *
FROM final
